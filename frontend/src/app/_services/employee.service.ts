...
export class EmployeeService{
  ...
  create(params: any) {
    return this.http.post<Employee>(baseUrl, params).pipe(
      map(employee => {
        // Ensure employee has isActive property for backward compatibility
        if (employee && employee.status && employee.isActive === undefined) {
  ...
  getAll() {
    return this.http.get<Employee[]>(baseUrl).pipe(
      map(employees => {
        // Ensure all employees have isActive property for backward compatibility
